...
function manageRow(data) {
	...
	$.each( data, function( key, value ) {
	  	rows = rows + '<tr>';
	  	rows = rows + '<td>'+value.postitle+'</td>';
	  	rows = rows + '<td>'+value.postdesc+'</td>';
	  	rows = rows + '<td>'+value.postcontent+'</td>';
	  	rows = rows + '<td data-id="'+value.postID+'">';
...
$("body").on("click",".remove-item",function(){
    ...
    $.ajax({
        dataType: 'json',
        type:'POST',
        url: 'http://thecreativekey.com/admin/' + 'deleteblog.php',
        data:{id:id}
    }).done(function(data){
        c_obj.remove();
...
$(".crud-submit-edit").click(function(e){
    ...
    if(title != '' && description != ''){
        $.ajax({
            dataType: 'json',
            type:'POST',
            url: 'http://thecreativekey.com/admin/'+form_action,
            data:{title:title, description:description,id:id,content:content}
        }).done(function(data){
            getPageData();
            $(".modal").modal('hide');

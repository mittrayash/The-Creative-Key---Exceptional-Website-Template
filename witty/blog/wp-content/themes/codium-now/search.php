...
<div id="container">
	<div id="content" class="">
		...
		<div class="dp100">
			<h1 class="page-title"><?php printf( __( 'Search Results for: %s', 'codium-now' ), '<span>' . get_search_query() . '</span>' ); ?></h1>
			<div class="hr"></div>
		</div>
		<?php if (have_posts()) : ?>  
		...
		<div class="one-third column posthome">
	       <div id="post-<?php the_ID() ?>" <?php post_class('dp100') ?>>
               ...
               <?php } ?>
               <h2 class="entry-title"><a href="<?php the_permalink() ?>" title="<?php printf(__('Link to %s', 'codium-now'), esc_html(get_the_title(), 1)) ?>" rel="bookmark"><?php the_title() ?></a></h2>
               <div class="clear"></div>
